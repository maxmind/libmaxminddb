...
static void test_record_rejected(const char *fixture,
    ...
    assert_true_desc(result.found_entry, "%s: entry found", desc);
    if (result.found_entry) {
        ...
        assert_int_equal_desc(
            ...
            desc);
        assert_true_desc(entry_data_list == NULL,
...
static void test_record_allowed(const char *fixture,
    ...
    if (result.found_entry) {
        ...
        int const status =
            MMDB_get_entry_data_list(&result.entry, &entry_data_list);
        assert_int_equal_desc(
            status, MMDB_SUCCESS, "%s: full decode succeeds", desc);
        assert_true_desc(
            entry_data_list != NULL, "%s: full decode returns a list", desc);
        ...
        assert_int_equal_desc(values,
                              expected_values,
                              "%s: decoded the expected number of values",
                              desc);
        assert_int_equal_desc(payload,
                              expected_payload,
...
static void test_per_call_state(void) {
    ...
    assert_true_desc(result.found_entry, "per-call state: entry found");
    if (result.found_entry) {
        for (int i = 1; i <= 2; i++) {
...
static void test_decoder_limit_error_message(void **UNUSED(state)) {
    assert_string_equal_desc(
        MMDB_strerror(MMDB_DECODER_LIMIT_ERROR),
        "The decoded data structure exceeds the configured resource limits",
        "decoder limit status has a distinct error message");
}

static void test_decoder_limits(void **UNUSED(state)) {
...
int main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(test_decoder_limit_error_message),
        cmocka_unit_test(test_decoder_limits),
    };
