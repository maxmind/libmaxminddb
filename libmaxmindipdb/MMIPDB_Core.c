...
_fddecode_key(IPDB_s * ipdb, int offset, struct IPDB_Decode_Key *ret_key)
{
    ...
    if (type == IPDB_DTYPE_PTR) {
        int psize = (ctrl >> 3) & 3;
        int new_offset;
        if (_read(fd, &b[0], psize + 1, segments + offset) != IPDB_SUCCESS)
            return IPDB_IOERROR;
        switch (psize) {
        case 0:
...
_fdlookup_by_ipnum(IPDB_s * ipdb, uint32_t ipnum, struct IPDB_Lookup *result)
{
    ...
    if (rl == 6) {
        for (depth = 32 - 1; depth >= 0; depth--, mask >>= 1) {
            if (_read
                (fd, &b[0], 3,
                 offset * rl + ((ipnum & mask) ? 3 : 0)) != IPDB_SUCCESS)
                return IPDB_IOERROR;
        ...
        for (depth = 32 - 1; depth >= 0; depth--, mask >>= 1) {
            ...
            if (ipnum & mask) {
                if (_read(fd, &b[0], 4, byte_offset + 3) != IPDB_SUCCESS)
                    return IPDB_IOERROR;
        ...
        for (depth = 32 - 1; depth >= 0; depth--, mask >>= 1) {
            if (_read
                (fd, &b[0], 4,
                 offset * rl + ((ipnum & mask) ? 4 : 0)) != IPDB_SUCCESS)
                return IPDB_IOERROR;
            offset = _get_uint32(b);
            if (offset >= segments) {
                result->netmask = 32 - depth;
