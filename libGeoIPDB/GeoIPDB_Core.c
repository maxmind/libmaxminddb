...
void GeoIPDB_free_all(GeoIPDB * gi)
{
	if (gi) {
		if (gi->fd >= 0)
			close(gi->fd);
...
static int
_fdlookup_by_ipnum_128(GeoIPDB * gi, struct in6_addr ipnum,
		       struct GeoIPDB_Lookup *result)
{
	int segments = gi->segments;
	int offset = 0;
	int byte_offset;
...
static int
_lookup_by_ipnum_128(GeoIPDB * gi, struct in6_addr ipnum,
		     struct GeoIPDB_Lookup *result)
{
	int segments = gi->segments;
	int offset = 0;
...
static int _init(GeoIPDB * gi, char *fname, uint32_t flags)
{
	struct stat s;
	int fd;
	uint8_t *ptr;
...
GeoIPDB *GeoIPDB_open(char *fname, uint32_t flags)
{
