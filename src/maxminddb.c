...
LOCAL int MMDB_open_internal(MMDB_s *const mmdb);
LOCAL void MMDB_init(MMDB_s *const mmdb);
...
cleanup:
    if (MMDB_SUCCESS != status) {
        ...
        status = MMDB_open_internal(mmdb);
    }

    return status;
}

int MMDB_open_memory(const uint8_t* data, ssize_t size, MMDB_s *const mmdb)
{
    MMDB_init(mmdb);
    ...
    mmdb->file_size = size;
    mmdb->file_content = data;

    return MMDB_open_internal(mmdb);
}
