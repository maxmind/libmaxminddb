...
void test_negative_indent(void **UNUSED(state)) {
    ...
    if (result.found_entry) {
        ...
        assert_int_equal_desc(
            status, MMDB_SUCCESS, "get_entry_data_list succeeded");

        if (MMDB_SUCCESS == status && entry_data_list) {
            FILE *devnull = fopen("/dev/null", "w");
            if (!devnull) {
                fail_msg("could not open /dev/null");
            }
...
int main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(test_negative_indent),
    };
