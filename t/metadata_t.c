...
void test_metadata(MMDB_s *mmdb, const char *mode_desc) {
    assert_int_equal_desc(
        mmdb->metadata.node_count, 163, "node_count is 163 - %s", mode_desc);
    assert_int_equal_desc(
        mmdb->metadata.record_size, 24, "record_size is 24 - %s", mode_desc);
    assert_int_equal_desc(
    ...
    assert_true_desc(mmdb->metadata.build_epoch >= expect_epoch,
                     "build_epoch is at least %llu",
                     (unsigned long long)expect_epoch);

    assert_int_equal_desc(mmdb->metadata.binary_format_major_version,
                          2,
                          "binary_format_major_version is 2 - %s",
                          mode_desc);
    assert_int_equal_desc(mmdb->metadata.binary_format_minor_version,
                          0,
                          "binary_format_minor_version is 0 - %s",
                          mode_desc);

    assert_int_equal_desc(
        mmdb->metadata.languages.count, 2, "found 2 languages - %s", mode_desc);
    assert_string_equal_desc(mmdb->metadata.languages.names[0],
                             "en",
                             "first language is en - %s",
                             mode_desc);
    assert_string_equal_desc(mmdb->metadata.languages.names[1],
                             "zh",
    ...
    for (uint16_t i = 0; i < mmdb->metadata.description.count; i++) {
        ...
        if (strncmp(language, "en", 2) == 0) {
            assert_string_equal_desc(
                description, "Test Database", "en description");
        } else if (strncmp(language, "zh", 2) == 0) {
            assert_string_equal_desc(
                description, "Test Database Chinese", "zh description");
        } else {
            fail_msg("found unknown description in unexpected language - %s",
                     language);
        }
    }

    assert_int_equal_desc(mmdb->full_record_byte_size,
                          6,
...
test_languages_value(MMDB_entry_data_list_s *entry_data_list) {
    ...
    assert_int_equal_desc(languages->entry_data.type,
                          MMDB_DATA_TYPE_ARRAY,
                          "'languages' key's value is an array");
    assert_int_equal_desc(languages->entry_data.data_size,
                          2,
    ...
    assert_string_equal_desc(lang0, "en", "first language is en");
    free(lang0);
    ...
    assert_string_equal_desc(lang1, "zh", "second language is zh");
    free(lang1);
...
test_description_value(MMDB_entry_data_list_s *entry_data_list) {
    ...
    assert_int_equal_desc(description->entry_data.type,
                          MMDB_DATA_TYPE_MAP,
                          "'description' key's value is a map");
    assert_int_equal_desc(description->entry_data.data_size,
                          2,
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (strcmp(key_name, "en") == 0) {
            assert_string_equal_desc(description,
                                     "Test Database",
            ...
            fail_msg("unknown key found in description map - %s", key_name);
        }
...
void test_metadata_as_data_entry_list(MMDB_s *mmdb, const char *mode_desc) {
    ...
    assert_int_equal_desc(
        first->entry_data.data_size, 9, "metadata map has 9 key/value pairs");
    ...
    while (1) {
        ...
        if (strcmp(key_name, "node_count") == 0) {
            MMDB_entry_data_list_s *value = entry_data_list =
                entry_data_list->next;
            assert_int_equal_desc(
                value->entry_data.uint32, 163, "node_count == 163");
        } else if (strcmp(key_name, "record_size") == 0) {
            MMDB_entry_data_list_s *value = entry_data_list =
                entry_data_list->next;
            assert_int_equal_desc(
                value->entry_data.uint16, 24, "record_size == 24");
        } else if (strcmp(key_name, "ip_version") == 0) {
            MMDB_entry_data_list_s *value = entry_data_list =
                entry_data_list->next;
            assert_int_equal_desc(
                value->entry_data.uint16, 4, "ip_version == 4");
        } else if (strcmp(key_name, "binary_format_major_version") == 0) {
            MMDB_entry_data_list_s *value = entry_data_list =
                entry_data_list->next;
            assert_int_equal_desc(value->entry_data.uint16,
                                  2,
            ...
            fail_msg("unknown key found in metadata map - %s", key_name);
        }
...
static void test_metadata_from_database(void **UNUSED(state)) {
    for_all_modes(&run_tests);
}

int main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(test_metadata_from_database),
    };
