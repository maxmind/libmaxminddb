...
LOCAL int fd_for_database(const char *filename, uint32_t flags)
{
    ...
    if (flags & MMDB_MODE_USE_SHARED_MEMORY) {
        ...
        (void)realpath(filename, abs_path);

        snprintf(shm_name, 255, "/maxminddb-%u", ftok(abs_path, 1234567));
        fprintf(stderr, "shm name = %s\n", shm_name);

        int fd = shm_open(shm_name, O_CREAT | O_RDONLY, S_IRUSR | S_IRGRP);
        if (-1 == fd) {
            return fd;
        }
