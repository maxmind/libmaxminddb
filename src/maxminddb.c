...
LOCAL const uint8_t *find_metadata(const uint8_t *file_content,
                                   ssize_t file_size);
LOCAL int read_metadata(MMDB_s *mmdb, const uint8_t *metadata);
...
uint16_t MMDB_open(const char *filename, uint32_t flags, MMDB_s *mmdb)
{
    ...
    const uint8_t *metadata = find_metadata(file_content, size);
    if (NULL == metadata) {
        return MMDB_INVALID_METADATA_ERROR;
    }

    int status = read_metadata(mmdb, metadata);
    if (MMDB_SUCCESS != status) {
        free_mmdb_struct(mmdb);
        return status;
    }
                                   ...
                                   ssize_t file_size)
{
    ...
    return (const uint8_t *)(search_area + strlen(METADATA_MARKER));
}
